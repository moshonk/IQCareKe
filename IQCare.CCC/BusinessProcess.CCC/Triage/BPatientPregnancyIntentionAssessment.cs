...
namespace BusinessProcess.CCC.Triage
{
    ...
    public class BPatientPregnancyIntentionAssessment : ProcessBase, IPatientPregnancyIntentionAssessmentManager
    {
        ...
        public int DeletePregnancyIntentionAssessment(int Id)
        {
            using (UnitOfWork _unitOfWork = new UnitOfWork(new GreencardContext()))
            {
                var pia = _unitOfWork.PatientPregnancyIntentionAssessmentRepository.GetById(Id);
                _unitOfWork.PatientPregnancyIntentionAssessmentRepository.Remove(pia);
                Result = _unitOfWork.Complete();
                _unitOfWork.Dispose();
                return Result;
            }
        ...
        public int CheckPregnancyIntentionAssessmentExists(int patientId)
        {
            using (UnitOfWork _unitOfWork = new UnitOfWork(new GreencardContext()))
            {
                var pia = _unitOfWork.PatientPregnancyIntentionAssessmentRepository.FindBy(x => x.PatientId == patientId & !x.DeleteFlag)
                    .Select(x => x.Id)
                    .FirstOrDefault();
                _unitOfWork.Dispose();
                return Convert.ToInt32(pia);
            }
        }

        List<PatientPregnancyIntentionAssessment> IPatientPregnancyIntentionAssessmentManager.GetPregnancyIntentionAssessment(int patientId, DateTime visitDate)
        {
            using (UnitOfWork _unitOfWork = new UnitOfWork(new GreencardContext()))
            {
                var piaList = _unitOfWork.PatientPregnancyIntentionAssessmentRepository.FindBy(x => x.PatientId == patientId & x.VisitDate == visitDate & !x.DeleteFlag).ToList();
                _unitOfWork.Dispose();
                return piaList;
            }
        }
        ...
        public List<PatientPregnancyIntentionAssessment> GetPregnancyIntentionAssessment(int patientId)
        {
            using (UnitOfWork _unitOfWork = new UnitOfWork(new GreencardContext()))
            {
                var piaList = _unitOfWork.PatientPregnancyIntentionAssessmentRepository.FindBy(x => x.PatientId == patientId & !x.DeleteFlag).ToList();
                _unitOfWork.Dispose();
                return piaList;
            }
        }
        ...
        public int CheckPregnancyIntentionAssessmentExists(int patientId, DateTime visitDate)
        {
            using (UnitOfWork _unitOfWork = new UnitOfWork(new GreencardContext()))
            {
                var pia = _unitOfWork.PatientPregnancyIntentionAssessmentRepository.FindBy(x => x.PatientId == patientId & x.VisitDate == visitDate & !x.DeleteFlag)
                    .Select(x => x.Id)
                    .FirstOrDefault();
                _unitOfWork.Dispose();
                return Convert.ToInt32(pia);
