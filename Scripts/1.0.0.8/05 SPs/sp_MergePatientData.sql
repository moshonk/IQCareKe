...
BEGIN
	...
	SELECT 
		[Name] as TableName 
	INTO #tmpTables
	FROM sys.all_objects WHERE type= 'U' AND SCHEMA_ID = 1 AND [Name] NOT IN ('Patient', 'mst_Patient', 'person', 'PatientIdentifier', 'Lnk_PatientProgramStart', 'ARTPataient', 'PatientEnrollment', 'IQCareLineList')

	SELECT @tableName = min(TableName) FROM #tmpTables
	WHILE @tableName IS NOT NULL
	BEGIN
		SELECT top 1 @PatientFk = [name] FROM sys.columns WHERE object_id = OBJECT_ID(@tableName) AND [name] in ('PatientId','Ptn_pk','PatientPk','PtnPk','Patient_Pk')
